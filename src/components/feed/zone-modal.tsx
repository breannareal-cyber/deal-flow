...
const MODALS: Record<ZoneKey, { title: string; accent: string; body: React.ReactNode }> = {
  CRITERIA_MATCH: {
    ...
    body: (
      <>
        ...
        <div className="flex flex-col gap-px rounded-sm overflow-hidden" style={{ backgroundColor: '#252525' }}>
          {BUY_BOX_ROWS.map(([dim, target]) => (
            <div key={dim} className="grid grid-cols-[140px_1fr] gap-4 p-3" style={{ backgroundColor: '#161616' }}>
              <span className="text-xs font-semibold tracking-wide uppercase" style={{ color: '#e8715a' }}>{dim}</span>
              <span className="text-sm" style={{ color: '#ccc' }}>{target}</span>
            </div>
          ))}
        </div>
        <div className="mt-5 rounded-sm border p-4" style={{ borderColor: '#d4a84733', backgroundColor: '#1a1a1a' }}>
          <p className="text-xs font-semibold tracking-widest uppercase mb-2" style={{ color: '#d4a847' }}>⚠️ Known open tension</p>
    ...
    body: (
      <>
        <p className="text-sm leading-relaxed mb-4" style={{ color: '#999' }}>
          These are <strong style={{ color: '#fff' }}>genuine water / environmental businesses</strong> — your sector
          bullseye — that fall <strong style={{ color: '#fff' }}>outside your spend criteria</strong>. Usually that
  ...
  SPEND_OUTSIDE_WATER: {
    ...
    body: (
      <>
        <p className="text-sm leading-relaxed mb-4" style={{ color: '#999' }}>
          These sit <strong style={{ color: '#fff' }}>within your spend range ($300K–$1.5M EBITDA)</strong> but{' '}
          <strong style={{ color: '#fff' }}>outside the water thesis</strong>. They&rsquo;re here as a deliberate
          wildcard — capped at 3, and filtered hard for quality.
        </p>
        <p className="text-sm leading-relaxed mb-4" style={{ color: '#999' }}>
          Only <strong style={{ color: '#fff' }}>genuinely enduring, recession-resistant small businesses</strong> the
...
export function ZoneModal({ zone, onClose }: { zone: ZoneKey; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-5 border-b sticky top-0" style={{ borderColor: '#1e1e1e', backgroundColor: '#111' }}>
          <h2 className="text-lg font-black tracking-tight" style={{ color: m.accent }}>{m.title}</h2>
          <button onClick={onClose} className="text-sm transition-colors hover:text-white" style={{ color: '#666' }}>✕ Close</button>
        </div>
